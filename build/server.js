...
async function getMailClient() {
  const accunt = await import_nodemailer.default.createTestAccount();
...
async function confirmTrip(app2) {
  app2.withTypeProvider().get("/trips/:tripId/comfirm", {
    schema: {
    ...
    "/participants/:participantId",
    {
      schema: {
        params: import_zod9.z.object({
          participantId: import_zod9.z.string().uuid()
        })
      }
    },
    async (request) => {
    ...
    "/trips/:tripId/participants",
    {
      schema: {
        params: import_zod13.z.object({
          tripId: import_zod13.z.string().uuid()
        })
      }
    },
    async (request) => {
